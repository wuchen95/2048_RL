...
class UI:
    ...
    def reset(self, to_show):
        self.game.new_game()
        if(to_show):
            self.show()
        # return observation
        return (np.array(self.game.field).flatten())  # 二维变一维展开 需要的格式
    def step(self, action, to_show):
        old_score = self.game.score
        self.game.move(action)
        new_score = self.game.score
        done = self.game.is_finished()
        s_ = np.array(self.game.field).flatten()
    ...
    def show(self):
        ...
        iss_finished = self.game.is_finished()
        if iss_finished:
